...
def run(argv=None):
  ...
  args = [
    "--runner=PortableRunner", "--job_endpoint=localhost:8099", "--environment_type=LOOPBACK", "--max_bundle_size=1000",
  ]
  if argv:
    args.extend(argv)
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    lines = p | beam.Create([
        f"Hello, world {i}! " + chr(ord('a') + (i % 26)) * (1024*1024*2 - 20) for i in range(1000)
    ])

    # Count the occurrences of each word.
    counts = (
        lines
        | 'PairWithOne' >> beam.Map(lambda x: (x, x))
